...
-- Create improved function that matches frontend logic
...
BEGIN
  ...
  IF v_furthest_stage_index = 6 THEN
    -- Only mark as done if Go Live is completed
    v_status := 'done';
...
-- Re-sync all existing customers by updating them directly
UPDATE customers 
SET updated_at = updated_at 
WHERE id IN (SELECT DISTINCT customer_id FROM lifecycle_stages);